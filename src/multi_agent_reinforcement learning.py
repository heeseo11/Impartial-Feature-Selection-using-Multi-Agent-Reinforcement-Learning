...
from tensorflow.keras.layers import Dense, GRU, BatchNormalization, Input, Flatten, Concatenate, Dropout, RepeatVector
...
EMR = pd.read_excel("./Data/EMR.xlsx")
...
for episode in range(num_episodes):
    ...
    features = [i for i, act in enumerate(actions) if act == 1]
    R = get_reward(features)
    reward_store[len(features)-1] = max(reward_store[len(features)-1], R)
    reward_store_array.append(["R", features, R])
    all_rewards.append(R)

    initial_action = actions[:]
    for agent in range(num_agents):
        initial_action = actions[:]
        initial_action[agent] = 1 - initial_action[agent]
        features = [i for i, act in enumerate(initial_action) if act == 1]
        C_agent = get_reward(features) - R + (R / len(features) if agent == 1 else 0)
        agent_reward_store_array.append(["A_R", features, R, C_agent])
        Q_values[agent][initial_action[agent]] += alpha * (C_agent - Q_values[agent][initial_action[agent]])
